...
def plot(file: string):
    ...
    observations, actions, rewards, win_percentage = run(env, agent, act=True)
...
def run(env, agent, n_episodes=1000, act=False):
    rewards = []
    observations = []
    actions = []
    won = 0
...
for i in range(1, 1 + 1):
    for ii in range(1, 1 + 1):
        plot(f'../results/TD3_{gym_name}_{test_name}_{i}_{ii}')
