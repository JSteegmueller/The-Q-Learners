...
# for rendereing
env = laserhockey.hockey_env.HockeyEnv(keep_mode=True)
...
'''
    ...
    def step(self,
        ...
        try:
            ...
            # rendering
            state = np.array(ob)
            CENTER_X = laserhockey.hockey_env.CENTER_X 
            CENTER_Y = laserhockey.hockey_env.CENTER_Y
    ...
    def game_done(self,
        ...
        if self.verbose:
                ...
                self.last_1000_games += [1] * result["games_won"] + [0] * (4 - result["games_won"])
                while len(self.last_1000_games) > 1000:
                    self.last_1000_games.pop(0)
                
                plt.gca().cla() 
                plt.plot(self.last_1000_games, ".", label="games", alpha=0.3)
                mov_avg = [sum(self.last_1000_games[i:i+30])/30 for i in range(len(self.last_1000_games)-30)]
                x_mov_avg = [i+30 for i in range(len(mov_avg))]
                plt.plot(x_mov_avg, mov_avg, label="moving average")
                plt.legend()
                plt.title("win rate over time")
                plt.grid()
                plt.draw()
                print("30 game win rate:", sum(self.last_1000_games[-30:])/30)
